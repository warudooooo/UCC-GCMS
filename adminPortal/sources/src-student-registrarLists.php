        ...
        foreach ($data as $row) {
            ...
            if ($count > 0) {
                ...
                if (mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM tbl_studentsregistrar WHERE studentNumber='{$studentNumber}'")) > 0) {
                    ...
                    $msg = "<div class='eml' style='display: inline-block; text-align: center; color: #38b000;'><h3>The system already contains some of the Students's data, so the data was updated.</h3></div>";
                } else {
...
if(isset($_POST['addStudent'])){
    ...
    if(mysqli_num_rows(mysqli_query($mysqli, "SELECT * FROM tbl_studentsregistrar WHERE studentNumber = '{$_POST['studentNumber']}'")) > 0){
        ...
        $msg = '<div class="eml" style="display: inline-block; text-align: center; color: crimson;"><h3>Please add "Status" to the student. </h3></div>';
    }else{
...
if(isset($_POST['editStudent'])){
    ...
    if($admPassword != $curPassword){
        $msg = '<div class="eml" style="display: inline-block; text-align: center; color: crimson;"><h3>Incorrect password.</h3></div>';
    } else if($_POST['studentStatus'] == ""){
        $msg = '<div class="eml" style="display: inline-block; text-align: center; color: crimson;"><h3>Please add "Status" to the student. </h3></div>';
    }else{
...
Phone: $studentPhone";
                    ...
                    VALUES('$admName','$before','$after','')";
                    $runActivity = mysqli_query($mysqli, $activity);

                    $msg = '<div class="eml" style="display: inline-block; text-align: center; color: #457b9d; "><h3>Edit Success.</h3></div>';
    }
}